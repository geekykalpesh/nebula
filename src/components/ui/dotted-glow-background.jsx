...
export const DottedGlowBackground = ({
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      // Avoid resizing if dimensions haven't changed to prevent flicker
      if (el.width !== Math.floor(currentWidth) || el.height !== Math.floor(currentHeight)) {
        el.width = Math.max(1, Math.floor(currentWidth));
        el.height = Math.max(1, Math.floor(currentHeight));
    ...
    const draw = (now) => {
      ...
      const { width, height } = container.getBoundingClientRect();
      // Ensure canvas is tailored to current size before clearing
